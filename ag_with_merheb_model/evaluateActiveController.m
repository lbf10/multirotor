function evaluateActiveController(filename)
...
%     foldername = strrep(foldername,' ','_');
    ...
    parfor it = 1:numberOfOptions
        ...
        numberOfMonteCarlo = 10;
        combinedFitness = 0;
        metrics = [];
        for jt = 1:numberOfMonteCarlo
    ...
    %         multirotor.addCommand(failure,step:step:endTime/2-0.000001);
            ...
            try
                ...
                combinedFitness = combinedFitness + 1*(1-metrics.simulationSuccess)+metrics.RMSPositionError+real(metrics.RMSAngularError)+metrics.RMSPower/500;
            catch
                combinedFitness = combinedFitness + 10;
            end
            metrics(jt) = multirotor.metrics();
        end
        combinedMetrics = struct();
        combinedMetrics.simulationSuccess = mean([metrics(:).simulationSuccess]);
        combinedMetrics.simulationEndError = mean([metrics(:).simulationEndError]);
        combinedMetrics.missionSuccess = mean([metrics(:).missionSuccess]);
        combinedMetrics.missionMetricPrecision = mean([metrics(:).missionMetricPrecision]);
        combinedMetrics.missionAngularPrecision = mean([metrics(:).missionAngularPrecision]);
        combinedMetrics.isStable = mean([metrics(:).isStable]);
        combinedMetrics.pathLength = mean([metrics(:).pathLength]);
        combinedMetrics.desiredPathLength = mean([metrics(:).desiredPathLength]);
        combinedMetrics.pathLengthRatio = mean([metrics(:).pathLengthRatio]);
        combinedMetrics.pathTimeRatio = mean([metrics(:).pathTimeRatio]);
        combinedMetrics.averageSpeed = mean([metrics(:).averageSpeed]);
        combinedMetrics.maxSpeed = mean([metrics(:).maxSpeed]);
        combinedMetrics.meanPositionError = mean([metrics(:).meanPositionError]);
        combinedMetrics.RMSPositionError = mean([metrics(:).RMSPositionError]);
        combinedMetrics.maxPositionError = mean([metrics(:).maxPositionError]);
        combinedMetrics.meanAngularError = mean([metrics(:).meanAngularError]);
        combinedMetrics.RMSAngularError = mean([metrics(:).RMSAngularError]);
        combinedMetrics.maxAngularError = mean([metrics(:).maxAngularError]);
        combinedMetrics.energy = mean([metrics(:).energy]);
        combinedMetrics.RMSPower = mean([metrics(:).RMSPower]);
        combinedMetrics.maxPower = mean([metrics(:).maxPower]);
        option{8} = combinedMetrics;
        option{7} = combinedFitness/numberOfMonteCarlo;
        options(it,:) = option;
