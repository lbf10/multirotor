function evaluateFDD(simulationCase,filename,saveDir)
data = load(filename);
...
for it=0:12
    for jt=0:9
        options(it*10+jt+1,:) = [simulationCase, 1-it/100, 0];
    end
end
for it=13:42
    for jt=0:9
        options(it*10+jt+1,:) = [simulationCase, 1, (it-12)/10];
    end
end
...
parfor it = 1:numberOfOptions
    ...
    multirotor.configFDD(option{10},option{11});
...
samples = options;
save([saveDir,'/',foldername,'/evaluationFDDResult.mat'],'samples');
clear all
end
